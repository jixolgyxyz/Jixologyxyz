...
export function MemberHoverCard({ userId, name, email, roles, rect, onMouseEnter, onMouseLeave }: MemberHoverCardProps) {
  ...
  const aboutMe = userProfile?.sobreMi ?? '';
  ...
  return (
    ...
    >
      {loading ? (
        <p style={{ margin: 'auto', textAlign: 'center', color: 'var(--color-anchor-gray-1)', fontSize: '0.875rem' }}>
          Cargando...
        </p>
      ) : (
        <>
          {/* UserCard fills remaining space; overflow hidden clips long bios */}
          ...
          {/* Roles pinned to the bottom, always visible */}
          <div style={{
            flexShrink: 0,
            background: 'var(--color-white)',
            padding: '0.625rem 1.125rem 0.875rem',
            borderTop: roles.length > 0 ? '1px solid rgba(31, 54, 80, 0.08)' : 'none',
          }}>
            {roles.length > 0 && (
              <>
